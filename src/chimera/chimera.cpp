...
namespace Chimera {
    ...
    const char *Chimera::get_map_path() noexcept {
        if (this->p_map_path.size() == 0){
            const char *path = this->get_ini()->get_value("halo.map_path");
            if (path){
                this->p_map_path = path;
            }
            else {
                this->p_map_path = std::string(this->get_path()) + "maps";
            }
            CreateDirectory(this->p_map_path.data(), nullptr);
        }
        return this->p_map_path.data();
    }
